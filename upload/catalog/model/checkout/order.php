...
class ModelCheckoutOrder extends Model {
    ...
    public function getOrderApi($order_id)
    {
        $data = [];
        $toReturn = [];

        $query = Capsule::table('order as o')
            ->select( 'o.order_id', 'o.shipping_method', 'o.payment_method', 'o.total', 'o.date_added');

        if (!is_null($order_id)) {
            $query->where('o.order_id', $order_id);
        }

        $query->orderBy('o.date_added')
            ->get()
            ->each(function ($order) use (&$data, &$toReturn){

                $dateAddedFormatted = \Carbon\Carbon::createFromDate($order->date_added);
                $data['id'] = $order->order_id;
                $data['paymentMethod'] = $order->payment_method;
                $data['totalCents'] = str_replace(',','', number_format($order->total, 2)) * 100;
                $data['createdAt'] = $dateAddedFormatted->toISOString();

                Capsule::table('order_product as op')
                    ->where('op.order_id', $order->order_id)
                    ->orderBy('op.price')
                    ->each(function ($order_product) use (&$data){
                        $data['items'][] = [
                            'id' => $order_product->product_id,
                            'title' => $order_product->name,
                            'quantity' => $order_product->quantity,
                            'costCents' => str_replace(',','', number_format($order_product->price, 2)) * 100,
                        ];
                    });

                Capsule::table('order_total as ot')
                    ->where('ot.order_id', $order->order_id)
                    ->where('ot.code', 'shipping')
                    ->orderBy('ot.title')
                    ->each(function ($order_total) use (&$data){
                        $data['shippingMethod'] = [
                            'title' => $order_total->title,
                            'costCents' => str_replace(',','', number_format($order_total->value, 2)) * 100,
                        ];
                    });

                $toReturn[$order->order_id] = $data;
            });

        ksort($toReturn);

        return count($toReturn) > 1 ? $toReturn : reset($toReturn);
    }
